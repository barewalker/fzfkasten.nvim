...
cmd("FzfKastenPanel", function() require('fzfkasten').panel() end, { desc = "Fzfkasten: Panel -- the commands worth a menu, and the notes" })
cmd("FzfKastenNotePanel", function() require('fzfkasten').note_panel() end, { desc = "Fzfkasten: Pick a note, then open / backlinks / rename / delete it" })
cmd("FzfKastenRecent", function() require('fzfkasten').recent() end, { desc = "Fzfkasten: Notes by when they were last written, newest first" })
cmd("FzfKastenLinkToDaily", function() require('fzfkasten').link_to_daily() end, { desc = "Fzfkasten: Yank a link to this line and write it into today's daily note" })
...
cmd("FzfKastenGraphExport", function(o)
	require('fzfkasten').graph_export(tonumber(o.args))
end, { nargs = "?", desc = "Fzfkasten: Write the graph out as an HTML page (whole collection, or a depth around this note)" })
...
cmd("FzfKastenWeekNotes", function(o)
	require('fzfkasten').week_notes(o.args)
end, { nargs = "?", desc = "Fzfkasten: The notes dated in a week (this one, -1 for last, 2026-W37, or a date in it)" })
cmd("FzfKastenWeekDigest", function(o)
	require('fzfkasten').week_digest(o.args)
end, { nargs = "?", desc = "Fzfkasten: A week's notes laid out in one buffer: outline, opening lines, tasks finished" })
cmd("FzfKastenAgenda", function(o)
	require('fzfkasten').agenda(o.args)
end, { nargs = "?", desc = "Fzfkasten: The calendar for a week (this one, -1 for last, 2026-W37, a date in it) or a span of weeks (0..1)" })
...
cmd("FzfKastenBlockIds", function(o)
	require('fzfkasten').mint_ids(o.range > 0 and { line1 = o.line1, line2 = o.line2 } or nil)
end, { range = true, desc = "Fzfkasten: Mint a ^id on every task line (in the range, or the buffer) that has none" })
