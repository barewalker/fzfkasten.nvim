...
describe("health: what it says when something is wrong", function()
    ...
    it("errors when the Claude integration is on but its multiplexer is not installed", function()
        setup({ claude = { enabled = true, pane = { via = "herdr", cmd = "fzfkasten-no-such-mux" } } })
        record()
        health.check()
        assert.is_not_nil(reported("error", "`fzfkasten-no-such-mux` is not on PATH"))
    end)

    it("errors when claude.pane.via is not a multiplexer it knows", function()
        setup({ claude = { enabled = true, pane = { via = "screen" } } })
        record()
        health.check()
        assert.is_not_nil(reported("error", "not a multiplexer"))
    end)

    it("says which pane a remote send would go through", function()
        setup({ claude = { enabled = true, pane = { via = "herdr", host = "nuc" } } })
        record()
        health.check()
        -- ssh is what has to be here; the multiplexer is the far end's business.
        assert.is_not_nil(reported("ok", "herdr on nuc, via ssh"))
    end)

    it("points out that a pane will be asked for when none is configured", function()
        setup({ claude = { enabled = true, pane = { via = "herdr", host = "nuc" } } })
        record()
        health.check()
        assert.is_not_nil(reported("info", "asks which pane"))
    end)
    ...
    it("says nothing alarming when the Claude integration is off", function()
        ...
        record()
        health.check()
        assert.is_nil(reported("error", "Claude"))
